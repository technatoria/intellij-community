...
USER_HOME=-Duser.home=/home/ide_builder
if [ "x${JPS_BOOTSTRAP_PREPARE_SKIP_DOWNLOAD:-0}" != "x1" ] ; then
  # Download and compile jps-bootstrap
  "$JAVA_HOME/bin/java" -ea -Daether.connector.resumeDownloads=false "$USER_HOME" -jar "$JPS_BOOTSTRAP_COMMUNITY_HOME/lib/ant/lib/ant-launcher.jar" "-Dbuild.dir=$JPS_BOOTSTRAP_PREPARE_DIR" -f "$JPS_BOOTSTRAP_DIR/jps-bootstrap-classpath.xml"
fi
...
"$JAVA_HOME/bin/java" -ea -Xmx4g -Djava.awt.headless=true "$USER_HOME" -classpath "$JPS_BOOTSTRAP_PREPARE_DIR/jps-bootstrap.out.lib/*" org.jetbrains.jpsBootstrap.JpsBootstrapMain "--java-argfile-target=$_java_args_file" "$@"
